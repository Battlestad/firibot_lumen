...
class firibot extends Command
{
     ...
     * @var string
     */
    protected $signature = 'firibot:execute';
    ...
    private function log($text) {
        \Log::info($text);
        $this->line("[".Carbon::now()->format("Y-m-d H:i:s")."] ".$text);
    }
    ...
    public function handle()
    {
        while (true) {
            ...
            $market = "BTCNOK";// Which crypto-currency to trade in
            ...
            if ($orderData['type'] != "" && $orderData['price'] != 0 && $diffPercentage >= 1) {
